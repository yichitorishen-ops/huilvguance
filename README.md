...
Scheduled collection starts two hours before each Beijing-time target slot and records the original slot labels. If GitHub delays an early schedule until within two hours after the target slot, the run is still accepted for that target label:

- Monday-Friday: runs every 10 minutes from 04:07 through 07:57, 11:07 through 14:57, and 16:07 through 19:57; records 06:00, 13:00, 18:00
- Tuesday-Saturday: runs every 10 minutes from the prior day 22:07 through 01:57; records 00:00
...
Scheduled runs use the original cron time to decide the data slot, not the delayed runner start time. If GitHub starts a scheduled run after the target slot's two-hour grace window, the collection step skips writing data to avoid storing a stale price in the wrong slot.
